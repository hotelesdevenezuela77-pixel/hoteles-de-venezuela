...
async function run() {
  ...
  for (const r of regions) {
    const host = `aws-1-${r}.pooler.supabase.com`;
    ...
    for (const ip of ips) {
      ...
      try {
        ...
        console.log(`SUCCESS connected to pooler in region ${r} on aws-1!`);
        const res = await client.query("SELECT version()");
        console.log("Version:", res.rows[0].version);
        await client.end();
        return;
      } catch (err) {
        console.log(`Failed for ${r} on aws-1: ${err.message}`);
      }
    }
  }
  console.log("All aws-1 regions scanned.");
}
